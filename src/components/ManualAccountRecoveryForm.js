...
function ManualAccountRecoveryForm({
	initialSignaturaId = '',
	returnPath = '',
}) {
	...
	return (
		<main className="min-h-screen bg-[radial-gradient(circle_at_80%_10%,rgba(239,68,68,0.18),transparent_30%),linear-gradient(180deg,#020617,#071224)] px-4 py-10 text-white">
			<div className="mx-auto mb-8 flex max-w-3xl items-center justify-between">
				...
				<Link
					href={returnPath || '/account-recovery'}
					className="text-sm font-semibold text-red-200">
					{returnPath ? 'Back to ACCURA login' : 'Back'}
				</Link>
			</div>
			...
			<section className="mx-auto w-full max-w-3xl rounded-2xl border border-white/10 bg-slate-950/90 p-6 shadow-2xl">
				...
				{returnPath ? (
					<p className="mt-4 rounded-xl border border-amber-400/30 bg-amber-400/10 px-4 py-3 text-sm leading-6 text-amber-100">
						After Signatura restores access and you register a new trusted device,
						return to ACCURA login authorization to complete sign-in.
					</p>
				) : null}
...
export { ManualAccountRecoveryForm };
