	...
	if (located.source === 'prisma') {
		...
		return {
			...
			body: {
				...
				template_name: record.metadata?.credential?.templateName || null,
				template_version: record.metadata?.credential?.templateVersion || null,
				document_number: record.metadata?.credential?.documentNumber || record.externalId,
				public_fields: record.metadata?.credential?.publicFields || {},
